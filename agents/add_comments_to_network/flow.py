...
def main_flow():
    ...
    run_agent(
        system_prompt=(
            ...
            "When posting, always call get_challenge first, then solve_pow_challenge "
            "with the challenge hash, then use the returned solution with create_message "
            "or reply_to_message. Reply with a funny image generated by the tool get_image_from_prompt."
        ),
        user_prompt=(
            "Write a message on Adam Network (https://adam-network.up.railway.app) containing a funny image generated by the tool get_image_from_prompt. The image should be relevant to the Adam Network and likely to generate interest and trends related to the AI and LLMs."
        ),
    )
