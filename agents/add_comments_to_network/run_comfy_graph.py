...
def upload_images_to_workflow(
    prompt_workflow, image_mappings, server_address
):
...
def run_comfyui_workflow(
    ...
    def free_comfyui_memory():
        ...
        try:
            ...
            if response.status_code == 200:
                print(
                    "🧹 Requested ComfyUI to unload models and free memory."
                )
            else:
                print(
    ...
    if input_image_mappings:
        upload_images_to_workflow(
            prompt_workflow, input_image_mappings, SERVER_ADDRESS
        )
    elif input_image_path:
        server_filename = upload_image_to_comfy(
            input_image_path, SERVER_ADDRESS
        )
    ...
    for node_id, node in prompt_workflow.items():
        ...
        if node_id in prompt_modifications.keys():
            print(
                f"Node ID {node_id} has a prompt modification. Applying it."
            )
        ...
        if response.status_code == 200:
            ...
            if target_ext == ".wav" and source_ext != ".wav":
                ...
                try:
                    ...
                    print(
                        f"✅ Audio converted and saved to: {output_file_path}"
                    )
        ...
        try:
            ...
            while True:
                ...
                if msg_type == "executing":
                    ...
                    print(
                        "❌ ComfyUI execution error received from WebSocket."
                    )
                    break
            ...
            if (
                ...
                websocket_details = _format_comfy_execution_error(
...
def generate_image_from_prompt(
    ...
    prompt_modifications = {
        "6": lambda node: {
            **node,
            "inputs": {**node.get("inputs", {}), "text": prompt},
        }
    }
...
if __name__ == "__main__":
    ...
    if args.workflow == "i_edit_exported.json":
        ...
        if len(args.image) != 1:
            raise ValueError(
                "❌ i2v_exported.json requires exactly 1 --image argument."
            )
        ...
        if len(args.image) != 0:
            raise ValueError(
                "❌ tts_audio.json does not accept --image arguments."
            )
        if not args.prompt.strip():
            raise ValueError(
                "❌ tts_audio.json requires a non-empty --prompt."
            )
