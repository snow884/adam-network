...
Navigate to that site, find its submission form, and fill in the input fields using `fill_element` with only the Adam Network project facts listed in your system prompt (name, website, GitHub repo, description), looking up any missing details on the GitHub repo page if needed. Then submit the form using `click_element` or `press_key`.
